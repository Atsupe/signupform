/**
 * Generate an alert component basedon the passed state key
...
const renderAlert = (state = 'error') => {
    ...
    const messages = {
        error: '<b>Please use a valid email.</b> Like: carolamegatcher@gmail.com.',
        success: '<b>Yay! Thank you!</b> We have sent a confirmation link to your inbox.'
    };

    return `
    <figure class="alert" data-state="${state}">
    <svg class="alert_icon" xmlns="https://www.w3.org/2000/svg" aria-hidden="true" focusable="false" width="1em" viewBox="0 0 24 24">
        <path fill="currentColor" d="${iconPaths[state]}"/>
    </svg>
    <p class="alert_content">${messages[state]}</p>
    </figure>
    `;
};
